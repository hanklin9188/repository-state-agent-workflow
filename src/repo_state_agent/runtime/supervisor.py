    ...
    try:
        with RuntimeLock.for_root(root):
            while transition_count < options.max_transitions:
                ...
                if decision.action == ACTION_PAUSE:
                    ...
                    if response:
                        doctor_failure = _check_adapter_once(
                            adapter, store, summary, doctor_checked
                        )
                        doctor_checked = True
                        if doctor_failure:
                ...
                doctor_failure = _check_adapter_once(adapter, store, summary, doctor_checked)
                doctor_checked = True
                if doctor_failure:
                    return doctor_failure
...
def _check_adapter_once(
    ...
    store.append_event({"type": "adapter_doctor", **doctor.to_dict()})
    if doctor.ok:
        return None
